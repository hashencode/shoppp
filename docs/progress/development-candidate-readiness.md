...
| Required implementation completion | Not audited            | The [Fashion Store functional-integration plan](../plans/2026-08-11-001-feat-fashion-store-functional-integration-plan.md) is complete, including U12 and U8. The product-level selected capability set and every other owning plan still require reconciliation. | Every required active plan is `completed`, every selected U is complete in that plan, and focused evidence is linked. The candidate ledger does not copy the feature's unit table.                                       |
...
| DC2 — Deployed commerce and administration          | Blocked | The P0 commerce chain, IAM human/service boundaries, Stripe test order/refund, queues, rollback journeys, and complete Fashion U12/U8 staging acceptance have retained evidence. | After DC1, rerun the complete activation-target chain against the exact frozen candidate. Historical U12/U8 evidence is implementation completion, not DC2 candidate evidence.                            |
