...
A second refresh follow-up isolated the remaining visible shift to Swiper's `spaceBetween` handoff: server-rendered thumbnails started with a zero-pixel gap, then initialization wrote a 15px bottom margin on desktop or right margin on mobile. Uninitialized Product thumbnails now reserve the same direction-specific 15px margin that Swiper will own after initialization. Browser measurements retained identical thumbnail dimensions and a 15px gap on both sides of the handoff at 1440px and 390px. Dedicated JavaScript-disabled desktop/mobile geometry checks passed, and the production-static Product gallery matrix passed 10 applicable timing, selection, boundary, interruption, responsive, pointer, keyboard, touch, lightbox, and failure-recovery checks with 8 intentional viewport skips. The temporary relocation used to isolate an ignored Decor Store preview artifact during the Fashion Store build was restored after verification.
