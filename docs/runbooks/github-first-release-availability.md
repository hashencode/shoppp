...
## Event-driven steady-state review

This is an event-driven review for the project's single maintainer, not an owner roster,
responsibility matrix, escalation tree, or calendar. Open a review only when one of the material
triggers below occurs. Record the triggering observation, required access, checks performed,
evidence references, and one `keep`, `revise`, or `remove` decision. Close the review when the
decision is implemented and verified; reopen CI-U11 through a governed plan only when the current
GitHub-first boundary no longer remains truthful or safe.

| Trigger                                                  | Required checks                                                                                                                                                                                                                                                                          | Decision record                                                                                                                                                           | Shutdown or reopen condition                                                                                                                                                                                          |
| -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GitHub billing or control-plane change                   | With repository and Actions administration access, verify source reachability, workflow dispatch, hosted-runner start, environment approval, same-run artifact transport, and audit visibility. Classify the four availability states without inferring proof from workflow definitions. | Keep while a fresh exact-SHA hosted run can produce and consume governed proof; revise provider guidance when product behavior changes.                                   | Pause formal release immediately on billing, queue, control-plane, environment, or audit uncertainty. Reopen CI-U11 if the GitHub-first control plane can no longer enforce the existing release boundary.            |
| Artifact retention or access change                      | With Actions run and artifact access, verify declared retention by artifact class, unique SHA/run/attempt names, digest checks, least-privilege visibility, expiry behavior, and same-run-only consumption.                                                                              | Keep the declared GitHub retention boundary, revise the documented periods or access procedure, or remove an obsolete artifact class without weakening required evidence. | Stop artifact consumption on missing, expired, altered, inaccessible, or cross-run input. Reopen CI-U11 if required proof cannot be retained or regenerated by a fresh governed run.                                  |
| Staging recovery contract change                         | With protected staging environment plus Cloudflare Workers and D1 access, rerun R27 coverage: pre-mutation refusal, Worker/D1 baseline capture, post-deploy checks, rollback or forward reconciliation, and return to the prior safe staging state with production disabled.             | Keep the recovery recipe only after all refusal, deployment, reconciliation, and restored-state checks pass; otherwise revise or remove the unsafe path.                  | Disable staging mutation on any failed prerequisite or recovery step. Reopen CI-U11 after workflow, deployment, migration, receipt, or rollback behavior changes materially.                                          |
| Credential rotation, revocation, or suspected disclosure | With GitHub environment/secret administration and the relevant provider administration, verify staging/production separation, least privilege, authorized actors, rotation, emergency revocation, audit visibility, and absence of usable secrets in repository evidence.                | Keep scopes that remain minimal, revise bindings after routine rotation, or remove compromised/unused credentials and affected paths.                                     | Disable every affected credentialed job, revoke exposed material, invalidate affected evidence, and reopen CI-U11 if the credential or approval model changes.                                                        |
| Toolchain drift                                          | With repository, hosted-run, package-provider, and release-report access, compare the declared Bun, Linux x64, browser, font, system, action-runtime, lockfile, gate order, report schema, and observed attestation.                                                                     | Keep matched evidence; revise and replay the exact SHA after an intentional change; remove unsupported tooling only through the owning contract.                          | Invalidate affected validation/deployment evidence and pause release until drift is resolved and a fresh exact-SHA hosted run passes. Reopen CI-U11 when the hosted platform or release toolchain changes materially. |
| Workflow action-pin update                               | With repository and workflow-run access, verify every external action remains pinned to a full immutable SHA, review the upstream change and permissions, run workflow-contract tests, then obtain fresh exact-SHA hosted and required staging recovery evidence.                        | Keep the pin when unchanged, revise it only with reviewed fresh proof, or remove an unused action and its permissions.                                                    | Reject mutable or unreviewed refs and stop affected workflows. Reopen CI-U11 when a pin update changes checkout, artifact, identity, permission, or deployment behavior.                                              |

The inaugural full recovery drill is retained in
`docs/progress/ci-u11-github-first-steady-state-review.md`. It closes the initial review only; it is
not reusable release evidence. The absence of a periodic reminder does not reopen work. A failed
triggered review, observed compromise, or material change that invalidates the current contract
does.
