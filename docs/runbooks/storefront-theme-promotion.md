...
Fashion Store remains an isolated private-preview experiment under the `fashion-store` theme ID. Passing
...
- The original `fashion` source and the `fashion-store` implementation have matching commit-bound
...
- Fashion, Decor, Fashion Store, and production-fallback builds pass their existing static,
  interaction, accessibility, performance, and selected-theme isolation gates.
- Fashion Store alone contains the approved source cascade and reviewed jQuery/vendor capabilities;
...
If the human decision is to abandon the experiment, remove the `fashion-store` descriptor, manifest,
...
Until promotion is separately approved, rollback is branch-local removal of the Fashion Store
