---
title: Fashion Store Crafto Source-Parity Home - Plan
type: feat
status: completed
date: 2026-08-06
topic: fashion-store-crafto-source-parity
...
# Fashion Store Crafto Source-Parity Home - Plan

> **Status after execution (2026-08-06):** This is a historical execution plan and postmortem, not
> the reusable HTML reconstruction workflow. The user later selected `fashion-store` as the only
> retained storefront theme and explicitly removed `fashion` and `decor`, superseding the original
...
- **Initial scope:** Fashion home only. Collection, product, cart, checkout, content, and order pages remain on the existing themes and are not copied into `fashion-store` in this plan.
...
- **Landing strategy:** Keep `fashion-store` experimental until its gates pass. Promotion to `fashion`/`2.0.0` is a separate explicit decision, not an automatic rename in this plan.

## Product Contract

### Summary

This plan changes the reconstruction method, not the storefront platform. The current Fashion theme is a reviewed Vue/CSS reimplementation that intentionally imports only binary assets. `fashion-store` instead treats the Crafto Fashion page as an executable visual implementation: Nuxt renders source-equivalent markup and data, the original stylesheets load in their original order, and a client-only adapter initializes only the upstream visual capabilities needed by the home page.
...
- `37489af` introduced the source contract, motion/font/capture contracts, interaction lifecycle, and large fidelity matrix only after multiple repair rounds. These are intake prerequisites for Fashion Store, not final hardening tasks.
...
Fashion Store converts those lessons into preventive gates:
...
6. Fashion Store-authored CSS starts empty and is limited to documented Nuxt integration or accessibility adaptations; visual correction must first use the source DOM and cascade. Authored override growth is recorded as experiment evidence so a second handwritten theme cannot silently emerge.

### Execution Outcome and Postmortem

The mechanical source-preservation strategy was directionally correct, but the original acceptance
contract was incomplete. It recorded regions, selectors, ordering, and counts more strongly than
triggers, runtime dependencies, transient states, elapsed-time behavior, visible outcomes, and
source-absent content. Automated checks could therefore pass while the live page still omitted
source behavior.

The user review identified these concrete gaps:

| Observation | Root cause | Corrected acceptance rule |
| --- | --- | --- |
| The native cursor disappeared over the hero | Source CSS retained the custom-cursor trigger while its runtime-created replacement was omitted | Port the complete DOM/CSS/runtime capability or remove the trigger and verify the native cursor remains visible |
| New Arrival showed one full-width card and did not rotate | Slide count and runtime initialization were checked, but visible card count, card width, track displacement, and autoplay were not | Assert responsive visible-card geometry and timed index/transform changes, including loop and fallback behavior |
| Promotion and “fashion core” marquees did not move | Deterministic screenshots froze motion and no live temporal channel proved movement | Use a separate temporal test with real elapsed time; static capture cannot certify animation |
| Social rail and scroll progress were missing | DOM presence was treated as completion, while capture CSS hid the controls and source state classes were not fully reproduced | Keep controls visible in their named-state captures; assert source state class, opacity, geometry, scroll progress, and back-to-top outcome |
| Search and mini-cart appeared inert | The named-state matrix omitted them and generic anchor/router ownership replaced their source control semantics | Inventory every actionable element; test open, focus, close, URL stability, hover/focus behavior, and source content |
| “Product added to the preview cart.” appeared without source evidence | “Visible accessible feedback” in R5/U5/U8 conflicted with source-authoritative copy | Default implementation-only visible-text budget to zero; any visible deviation requires an approved exception |

The selected countermeasure is a source-derived behavior ledger for actionable, stateful, and
runtime-bearing elements. The canonical fields, completeness rules, three parity layers, and five
acceptance modes now live only in
[`docs/runbooks/source-equivalent-html-template-port.md`](../runbooks/source-equivalent-html-template-port.md).
This historical plan records why that countermeasure was selected; it is not a second normative
definition. Presence of DOM, a library global, an initializer call, or an `initialized` class remains
supporting evidence only—the acceptance result must be user-observable.

The original unit sequence also contained a dependency flaw: U8 expected the comparison pipeline
to reject defects before U3-U7, while U6 owned the generalized comparison descriptor that U8 needed.
Future plans must place the minimum reference-to-implementation comparison seam before the harness
self-test and defer generalization until a second current consumer exists.

The original repository boundary changed after implementation:

- The supplied source package now lives under `templates/` and is the project-local intake root.
- `fashion-store` is the retained theme rather than an experimental sibling pending a separate explicit
  promotion decision.
- `fashion` and `decor` were deleted by explicit user decision; their regression requirements and
  referenced files in the original units below are historical, not current gates.
- Current isolation verification must derive its theme list from the generated catalog/policy
  rather than hard-code deleted theme IDs.
- Reconstruction, platform integration, promotion, and legacy cleanup must be separate milestones
  in future plans even when the user ultimately requests all of them.

Regression coverage added after review must name at least: `search-open`, `search-dismissed`,
`cart-open`, `cart-dismissed`, `collection-moving`, `marquee-moving`, `social-visible`,
`scroll-progress-visible`, and `back-to-top`. The left social rail and right progress control are
independent acceptance regions, not footer decoration.

This completed plan does not own new implementation work. Any further behavior-ledger automation,
matrix-completeness enforcement, legacy-data migration, or architecture-document refresh requires
a separate current-state implementation plan based on the retained theme catalog.

See
[`docs/solutions/workflow-issues/html-source-parity-reconstruction-workflow-2026-08-06.md`](../solutions/workflow-issues/html-source-parity-reconstruction-workflow-2026-08-06.md)
for the reusable reasoning behind these corrections.

### Requirements

#### R1. Source authority and reproducibility

- Record the exact upstream files, source-relative paths, SHA-256 hashes, import date, and license/ownership note for every stylesheet, font, image, icon, and runtime file used by `fashion-store`.
...
- Register `fashion-store` as its own descriptor, manifest, preset, registry, fixture namespace, resource map, preview build target, and test target.
- Keep section types, fixture IDs, asset IDs, generated catalog entries, and Fashion Store-owned auxiliary CSS/data/capture hooks namespaced under `fashion-store`; preserve upstream Crafto class tokens unchanged so the original selectors continue to match.
- Do not mutate or alias the current `fashion` implementation to make the experiment pass.
- Preserve build-time theme selection: a Fashion Store preview may include Fashion Store source files, while production fallback, `fashion`, and `decor` builds must not.
...
- Render source-equivalent repeated content from `fashion-store` fixtures rather than using `v-html` or shipping the source HTML as an iframe.
...
- Preserve keyboard operation, focus visibility, accessible names, and menu state semantics when adapting source anchors to buttons or Nuxt links. Do not add source-absent visible feedback; use non-visual semantics or an approved visible-content exception.

#### R6. Evidence and acceptance

- Keep the source reference identity (`fashion`) separate from the implementation identity (`fashion-store`) in capture and reporting tools.
...
2. Add `fashion-store` to the generated catalog inputs and the build-time module allowlist.
3. Generate a private Fashion Store home snapshot and prepare the selected-theme module before Nuxt compilation.
4. Confirm the compiled preview imports only the `fashion-store` registry while normal production fallback imports no theme package.

#### F2. Import the source implementation

1. Read the approved manifest entry for `fashion-store`.
...
4. Copy the approved folder-shaped subset to `apps/storefront/app/themes/fashion-store/upstream/`.
...
1. Resolve the `fashion-store` home template and fixture binding through the existing theme engine.
...
3. Initialize only selectors found inside the Fashion Store root and record every created instance/listener.
...
1. Capture the original `fashion` source and the `fashion-store` implementation using the same viewport/state contract.
...
4. If all gates pass, record Fashion Store as promotion-eligible; otherwise keep it isolated and record the blocking diffs.
...
- **Given** the original Crafto Fashion home and a Fashion Store preview at 1440 CSS pixels,
...
- **Then** Nuxt records or handles the typed action, source visual state remains intact, no source-absent visible feedback appears, and no upstream AJAX or hard navigation occurs.

#### AE5. Reduced motion and runtime failure

- **Given** reduced motion is requested or a visual runtime cannot initialize,
- **When** the page loads,
- **Then** all content is visible in a stable static layout, internal navigation and commerce actions work, and the failure is recorded diagnostically without adding source-absent visible UI or trapping the user in `grid-loading` or hidden animation state.

#### AE6. Theme isolation

- **Given** separate `fashion-store`, `fashion`, `decor`, and production-fallback builds,
- **When** their generated output and runtime globals are inspected,
- **Then** Crafto source CSS/runtime is present only in the Fashion Store preview and no existing theme screenshot or interaction baseline changes.

### Success Criteria

- `fashion-store` is selectable through the existing signed preview/build flow and declares only the `home` template.
...
- `fashion-store` catalog/build/test tooling and source-to-implementation fidelity reporting.
...
`fashion-store` receives independent registry, manifest, preset, fixtures, resources, source provenance, scripts, and captures. This keeps rollback trivial and makes any asset/runtime leakage observable. A `themeVersion: 2.0.0` under the existing `fashion` ID is reserved for a later promotion decision because versioning alone would not isolate current users during the experiment.

#### KTD2. Keep source reference and implementation identity separate

Capture APIs must accept `referenceThemeId: fashion` and `implementationThemeId: fashion-store` as distinct fields. Generalizing a single `themeId` union to include `fashion-store` would incorrectly imply a non-existent `demo-fashion-store-store.html` and would mix artifact directories.

#### KTD3. Preserve stylesheet order and source-relative folders

The source loads vendor, icon, shared, responsive, and Fashion demo CSS in a deliberate cascade. Fashion Store imports those exact files in that order from a preserved `upstream/` tree. The styles remain global inside the selected Fashion Store build because scoping or CSS Modules would alter selectors; build-time theme isolation is the containment boundary.

#### KTD4. Render one source-faithful home section

The Fashion Store preset contains one page-level `fashion-store.home` section whose Vue component emits source top-level siblings as a fragment. This avoids inserting component wrappers between the header, sections, footer, and sticky elements and gives one lifecycle owner for client initialization. Data remains fixture-backed and validated through the existing theme engine.
...
Importer changes must be additive and policy-driven: source-code kinds are legal only for the explicit `fashion-store` allowlist. Tests continue to prove that `fashion` and `decor` reject CSS/JS/runtime imports. The experiment cannot weaken production-fallback isolation or turn arbitrary theme IDs into executable source paths.
...
sequenceDiagram
  ...
  participant Adapter as Fashion Store adapter
...
├── components/FashionStoreHome.vue
├── composables/useFashionStoreRuntime.ts
├── runtime/
│   ├── capabilities.ts
│   ├── loader.client.ts
│   └── lifecycle.ts
...
- **Import/security boundary:** `tools/storefront-theme-source-manifest.json` and `tools/import-storefront-theme.ts` currently enforce binary-only import. Fashion Store needs explicit source kinds without enabling caller-supplied executable paths generally.
- **Theme runtime:** Global CSS and runtime globals are acceptable only in a selected Fashion Store preview. Initialization and teardown must be scoped to the Fashion Store root and reversible at document level.
- **Rendering/data:** Existing contracts already accept `fashion-store` identifiers, but the new manifest/preset must declare only `home` and bind one page-level view model containing all source sections.
...
- Existing preview fixtures are acceptable as application data so long as Fashion Store preserves the source-visible content and ordering.

### Risks and Mitigations

| Risk | Impact | Mitigation |
| --- | --- | --- |
| Global source CSS affects generic app shell | Layout or accessibility controls drift | Include CSS only in the statically selected Fashion Store registry; add bundle/content isolation assertions; limit preview to the source-faithful page shell. |
| Full vendor bundle is large or exposes globals | Performance and collision risk | Measure Fashion Store separately, document required capabilities, load client-side only, and remove adapter-owned globals/instances where possible. Do not raise budgets for existing themes. |
...
- **Should Fashion Store reuse the current Fashion assets?** No cross-theme imports. It may contain byte-identical copies, but its own provenance and hashes keep the experiment independent and promotable.
...
- **Should passing Fashion Store replace Fashion?** No. Passing creates a promotion candidate only.
...
### U1. Register the isolated Fashion Store preview surface

- **Goal:** Make `fashion-store` a valid, home-only, build-time-selected preview without changing current theme output.
...
- **Approach:** Add an explicit descriptor and static module allowlist entry. Declare only `home`, a single `source-parity` preset, and one `fashion-store.home` section. Add preview/dev/build/test scripts without changing the default production path or deriving arbitrary import paths from user input.
- **Test scenarios:**
  - A signed Fashion Store home snapshot matching descriptor versions prepares a static import of `themes/fashion-store/registry`.
  - A Fashion Store snapshot declaring a non-home template or mismatched version is rejected.
  - Production fallback output contains no Fashion Store import.
  - Catalog generation is deterministic and detects manual generated-file drift.
- **Verification outcome:** A minimal placeholder Fashion Store home builds privately; `fashion` and `decor` selected-module tests remain byte-for-byte behaviorally unchanged.
...
- **Verification outcome:** A clean checkout can rebuild the Fashion Store upstream tree from the authorized source package and can independently verify it afterward.
...
- **Files:**
  ...
  - `apps/storefront/app/themes/fashion-store/composables/useFashionStoreRuntime.ts`
...
- **Test scenarios:**
  ...
  - The product action records exactly one typed Nuxt intent with no source-absent visible feedback, AJAX, cookie write, or hard navigation; any required accessibility status remains non-visual unless explicitly approved.
  - Fashion, Decor, and production-fallback artifacts contain no Fashion Store stylesheet/runtime marker or source hash.
...
- **Verification outcome:** With motion disabled, Fashion Store is a complete, stable, source-styled home rather than a component-library approximation.
...
- **Files:**
  - `apps/storefront/app/themes/fashion-store/composables/useFashionStoreRuntime.ts`
  - `apps/storefront/app/themes/fashion-store/runtime/capabilities.ts`
  - `apps/storefront/app/themes/fashion-store/runtime/loader.client.ts`
...
- **Files:**
  ...
  - `apps/storefront/e2e/fashion-store-theme.spec.ts`
- **Approach:** Classify each source anchor as navigation, external link, overlay trigger, state controller, scroll action, or commerce action before assigning ownership. Map genuine internal navigation to local Nuxt routes and commerce affordances to existing typed preview intents. Add only the minimal new intent shape if a source affordance cannot be represented today. Preserve source classes and visual children; document semantic anchor-to-button adaptations. Ensure menu expanded state, focus return, non-visual accessibility status where needed, and keyboard activation remain available independent of upstream runtime. Source-absent visible feedback is prohibited without an approved exception.
- **Test scenarios:**
...
- **Goal:** Compare the original Fashion source to Fashion Store without weakening existing evidence contracts.
...
- **Approach:** Introduce an explicit comparison descriptor containing reference and implementation IDs, artifact roots, selectors, states, viewports, and DPR. Add Fashion Store selectors for initial home, desktop mega menu, mobile menu, hero slides/progress, product hover/focus, search open/dismissed, cart open/dismissed, collection moving, marquee moving, social visible, scroll progress visible, back-to-top, and footer state. Keep static visual, temporal motion, interaction, scroll/fixed, and fallback capture modes separate; hiding or freezing a capability in static mode does not satisfy its live-state evidence. Compare the behavior ledger and fidelity matrix so any unrepresented source state fails.
- **Test scenarios:**
  - `fashion -> fashion-store` resolves `demo-fashion-store.html` for the reference and Fashion Store preview output for implementation.
  - A request for a `fashion-store` source filename is rejected rather than silently mapped.
...
- `bun tools/import-storefront-theme.ts --source="<crafto-html-root>" --theme=fashion-store --ownership-confirmed`
...
- `bun test apps/storefront/tests/fashion-store-runtime.test.ts apps/storefront/tests/theme-actions.test.ts`
- Expected: no format/type/test failures and current binary-only policy remains asserted.

### Fashion Store preview gates

- `bun run --cwd apps/storefront build:preview:fashion-store`
- `bun run --cwd apps/storefront test:fashion-store -- --workers=1`
...
- Capture source reference and implementation at 1440, 1024, 768, and 390 CSS pixels using the explicit `fashion -> fashion-store` descriptor.
- Generate the full-page report and named-state evidence for initial home, desktop mega menu, mobile menu, hero progress/slide, product hover/focus, `search-open`, `search-dismissed`, `cart-open`, `cart-dismissed`, `collection-moving`, `marquee-moving`, `social-visible`, `scroll-progress-visible`, `back-to-top`, and footer state.
...
- Inspect Fashion, Decor, Fashion Store, and production-fallback output for selected-theme source hashes/markers.
- Expected: current theme behavior and budgets do not change; Fashion Store source code appears only in its preview artifact; production fallback imports no theme.
...
- The source import is reproducible, hash-pinned, ownership-documented, and restricted to Fashion Store.
- Fashion Store renders the full Fashion home source structure and exact CSS cascade through Nuxt fixtures and routing.
...
- Script order in source: jQuery, vendor bundle, `main.js`; Fashion Store narrows the last layer to reviewed visual initialization.
...
This plan is independent of `docs/plans/2026-07-31-001-refactor-source-equivalent-fashion-decor-plan.md`. The earlier plan remains an accurate record of the reviewed reimplementation strategy and the then-current, now-historical `fashion`/`decor` policy. Current isolation rules derive from the generated theme catalog and source-equivalence policy described in this postmortem; Fashion Store tested a different implementation method without retroactively rewriting the earlier record.
