...
- KTD5 的步骤操作改为使用 Ant Design 默认 Button 外观和默认尺寸的站内锚点，继续保留链接语义、应用部署前缀和原有权限判断。
- R6 的顶部展示由本条替代：只使用一个 Statistic 显示自动检查通过数/固定总数；待处理、无法检查和无权检查仍保留在各检查项的文字状态中，不再分别统计到顶部。
- 同一修正记录了本地开发错误的原因：最新 Admin 请求 `/admin/settings/setup-guide` 时，旧的 `shoppp-api-fashion-staging` Worker 返回 404，属于前后端版本不一致；不是 13 项检查中的配置失败。用户于 2026-09-04 明确授权部署匹配 API 版本；该环境修复不推进 REL/DC/PG。
- 后续产品修正进一步明确：Statistic 标题表达“已通过检查项”的计数含义，顶部不再展示环境、检查时间、默认币种或配置保存时间。每个步骤只要包含未通过检查就自动展开，步骤内检查全部通过时默认折叠；管理员仍可手动展开。检查项使用带项目符号的语义化无序列表。
