...
- **Objective:** Return Shoppp to one long-lived primary worktree without treating Fashion Store and Decor Store as opposing products, losing uncommitted material, or deleting the Decor branch and commits that preserve ongoing template development.
...
- **Tail ownership:** This plan owns local worktree topology only. It does not integrate Decor code, change shared remote state, delete its branch, determine template support, complete Fashion Store or Decor Store work, or advance DC/PG.
...
- **Inherited baseline:** [Safe Local Branch and Worktree Integration](2026-08-05-001-refactor-safe-local-integration-plan.md) remains the historical authority for branch/ref recoverability, semantic preservation, and the prohibition on changing shared remote state during local cleanup.
...
Keep one long-lived Shoppp checkout for ordinary product development. Remove redundant detached and Decor checkouts after recoverability checks, while retaining the Decor branch and commits so its ongoing template work remains available and independently schedulable.
...
The worktrees are implementation containers, not product boundaries. `fashion-store` and `decor-store` can coexist in one repository and progress through separate U plans without blocking each other. The Decor branch's unique commits remain recoverable through retained local and remote refs; deleting its local checkout does not delete its template, history, branch, or development plan.
...
- R2. For each target, record its exact resolved path, HEAD, branch or detached state, porcelain status, and whether the HEAD is reachable from a retained local or remote ref.
...
- R6. Deleting the Decor worktree must not integrate Decor code, rewrite or delete its branch, or change its shared remote state.
...
- R8. This plan does not require a commit-by-commit Decor harvest, disposition ledger, time box, convergence branch, or choice of integration protocol. Future integration follows the Decor plan when product work calls for it; no publication workflow is a prerequisite for this cleanup.
...
- R12. No remote ref, tag, product requirement, template status, DC state, PG state, or material source change is deleted or promoted by this plan.

### Acceptance Examples

- AE1. **Decor checkout is removed without removing Decor work**
  - **Covers:** R2, R6-R8, R12.
  - **Given:** The Decor worktree is clean and its HEAD is reachable through retained local and remote refs.
  - **When:** The exact local worktree path is removed.
  - **Then:** The branch, commits, template plan, product status, and shared remote state remain unchanged and recoverable.
...
- Any future Decor code integration or publication.
- Deleting the Decor remote/local branch or changing published history.
- Old Fashion runtime deletion, which remains owned by the FRT plan.

#### Outside This Product's Identity

- Treating a worktree, branch, retained ref, or commit subject as a product, template, support matrix, activation target, or completion authority.
...
- KTD3. Remove the Decor checkout without deleting its branch, commits, or shared remote state. `(session-settled: user-approved — chosen over retaining a permanent Decor worktree or forcing its unique commits into the current branch during repository cleanup.)` Governs R2, R6-R8, R12.
...
flowchart TB
  ...
  F --> G["Retain Decor branch and commits"]
...
| Codex Decor | `0c2cdb86`, `codex/feat-decor-store-source-parity` | Clean; 14 unique commits; retained local and remote refs | Revalidate clean/reachable, remove checkout, retain branch, commits, and independent Decor plan |
...
- **Approach:**
  ...
  3. U2.3 verifies the Decor branch and unique commits remain reachable, the primary checkout is untouched, and the final worktree list contains only the intended long-lived checkout.
...
- **Test scenarios:**
  ...
  4. Final enumeration shows one long-lived primary worktree while the Decor branch and unique commits remain available.
...
- **Approach:**
  ...
  3. U3.3 records the one-worktree steady state without advancing Fashion Store, Decor Store, DC, or PG status.
- **Test scenarios:**
  1. Fashion Store work can proceed while Decor work remains on its retained branch.
...
- **Plan status:** Incomplete. The 2026-08-14 re-execution audit revoked the earlier completion claim because the required pre-removal raw manifests and removal commands were not retained.
- **U1:** Not complete. The current one-worktree topology and retained Decor refs were reverified, but the deleted dirty worktrees' exact tracked, untracked, and material ignored inventories cannot be reconstructed.
- **U2:** Not complete. The four checkouts are physically absent, but their destructive removal cannot be replayed or independently verified against the required pre-removal evidence. No removal was attempted during the re-execution audit.
- **U3:** Complete. One long-lived Shoppp checkout remains and the steady-state rules are in `AGENTS.md`; this observable outcome does not complete U1 or U2 retroactively.
- **Current unit:** None. The missing historical inputs no longer exist, so there is no safe executable step that can complete U1 or U2.
- **Next unit:** None under this historical plan. Any future temporary-worktree cleanup must begin as a new bounded execution against live targets and retain the required manifests before removal. Product development continues from the master plan's active `FS` pointer.
- **Blocker:** The discarded uncommitted and ignored content is irrecoverable from the current checkout and retained Git refs. Current-state evidence must not be substituted for the missing pre-removal evidence.
- **Evidence:** [Worktree Convergence Evidence](../progress/worktree-convergence.md).
- **Tail boundary:** WTC does not own Decor integration, Fashion Store completion, retired-runtime removal, candidate scope, DC, or PG.
...
| Decor preservation | Decor branch and unique commit set | Checkout removal leaves the retained refs reachable and does not integrate or reclassify Decor work |
...
- The Decor branch, commits, plan, and developing-template status remain intact and independently resumable.
