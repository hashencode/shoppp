...
- **Current parent/child stage:** `CI-GH-U3` — bind protected deployment to the exact source and
  outputs accepted by hosted validation. CI-GH-U1 reconciled authority; CI-GH-U2 implemented the
  reusable trusted-source hosted validation and attestation contract without changing the 17 gates
  or Lighthouse thresholds. `CI-U7.3` remains incomplete, and its missing Intel restore remains
  missing.
- **Next action:** Add exact-input refusal tests, then make protected deployment consume only the
  same-run source/report/attestation/deployable identity while preserving all environment,
  credential, confirmation, backup, human-access, receipt, and rollback gates. Do not remove old
  implementation before the replacement is operationally proven.
...
- **Tail:** `CI-GH-U3` through `CI-GH-U7`, then return to `CI-U8.3` for the GitHub-first
