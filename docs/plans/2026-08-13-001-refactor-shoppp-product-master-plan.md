...
- **Current parent/child stage:** Active `CI-GH-U4` — obtain complete pre-removal GitHub-hosted full
  ...
  missing.
- **Next action:** Commit and push the retained protected-preparation evidence, then run the resulting
  exact clean protected-default source through `deploy.yml` for immutable successor
  `staging-canonical-2026-08-27-ci-gh-u4-b`, with rollback rehearsal enabled and production disabled.
...
- **Blocker:** No external billing blocker remains. Controlled mismatch run `33045559474` was safely
  ...
  `staging-canonical-2026-08-27-ci-gh-u4-b`. The current dependency is now only its exact-source
  hosted/staging rollback rehearsal. Hosted validation run `33041884429` passed all 17
