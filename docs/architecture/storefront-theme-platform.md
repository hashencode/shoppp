...
Fashion 2 is an isolated experiment under the `fashion-2` identity. It is the only selected build
...
The Fashion 2 accessibility suite fixes semantic names, landmarks, and focus behavior without
...
- `apps/storefront/scripts/check-bundle-budget.ts` enforces JavaScript and selected-theme output
  isolation, including the independent Fashion 2 source-runtime allowance.
