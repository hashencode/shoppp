...
export async function finishD1Export(
  ...
  const response = await postExport(request, { current_bookmark: bookmark });
  if (!response.success || !response.result?.signed_url) {
    throw new Error("d1_export_not_ready");
  }
  const dump = await (request.fetcher ?? fetch)(response.result.signed_url);
...
export class D1BackupWorkflow extends WorkflowEntrypoint<ApiBindings> {
  override async run(event: Readonly<WorkflowEvent<unknown>>, step: WorkflowStep) {
    ...
    try {
      ...
      await step.do(
        "store-d1-export",
        { retries: { backoff: "exponential", delay: "30 seconds", limit: 20 } },
        () => finishD1Export(request, bookmark, this.env.BACKUP_BUCKET, objectKey),
      );
