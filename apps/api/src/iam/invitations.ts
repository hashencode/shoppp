...
const INVITATION_SELECT = `SELECT invitation.id, invitation.normalized_email,
  ...
  FROM admin_invitations invitation JOIN admin_roles role ON role.id = invitation.role_id`;
...
export async function createAdminInvitation(
  ...
  await loadAssignableRole(context.env.DB, principal, input.roleId);
  const now = new Date().toISOString();
...
export async function resendAdminInvitation(
  ...
  const job = prepareInvitationNotification(context.env.DB, {
    invitationId: id,
  ...
  await context.env.DB.batch([
    ...
    context.env.DB.prepare(
      ...
      `admin-invitation:${id}:v${updated.version}`,
    ),
    prepareAuditEvent(context.env.DB, {
      ...
      actorType: actorTypeForPrincipal(principal),
      id: crypto.randomUUID(),
      metadata: { after: { status: "pending", version: updated.version } },
      requestId: context.get("requestId"),
      result: "succeeded",
      targetId: id,
      targetType: "admin_invitation",
...
export async function revokeAdminInvitation(
  ...
  await recordAuditEvent(context.env.DB, {
    ...
    actorType: actorTypeForPrincipal(principal),
    id: crypto.randomUUID(),
    metadata: { after: { status: "revoked", version: input.expectedVersion + 1 } },
    requestId: context.get("requestId"),
    result: "succeeded",
    targetId: id,
    targetType: "admin_invitation",
