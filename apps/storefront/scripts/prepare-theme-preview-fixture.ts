...
import { fashion2Manifest } from "../app/themes/fashion-2/manifest";
...
export async function fashion2PreviewBuildInput(
  expectedOrigin: string,
): Promise<Extract<ExperienceBuildInput, { environment: "preview" }>> {
  const fixture = previewFixtureSchema.parse(
    JSON.parse(
      await readFile(resolve(import.meta.dir, "../fixtures/experience/fashion-2.json"), "utf8"),
    ),
  );
  return {
    environment: "preview",
    ...
    snapshot: {
      ...
      provenance: fashion2Manifest.provenance,
      resolvedTemplates: fashion2Preset.templates,
      themeId: fashion2Manifest.id,
...
async function main(): Promise<void> {
  ...
  if (themeId !== "fashion" && themeId !== "decor" && themeId !== "fashion-2") {
  ...
  const input =
    themeId === "fashion"
      ...
      : themeId === "fashion-2"
        ? await fashion2PreviewBuildInput("https://preview.example.test")
