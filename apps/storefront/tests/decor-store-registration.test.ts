...
describe("Decor Store U2 registration", () => {
  ...
  test("prepares only readiness-gated fixtures and one isolated static registry import", async () => {
    ...
    expect(input.snapshot.bindings).toHaveLength(6);
    expect(input.snapshot.bindings).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          fixtureId: "decor-store-cart",
          instanceId: "decor-store-cart",
        }),
        expect.objectContaining({
          fixtureId: "decor-store-checkout",
          instanceId: "decor-store-checkout",
        }),
      ]),
    );
  ...
  test("registers dedicated commands and leaves preview prerendering selected-theme driven", async () => {
    ...
    expect(nuxtConfig).toContain("previewExperienceInput");
    expect(nuxtConfig).toContain("resolveStorefrontPrerenderRoutes");
