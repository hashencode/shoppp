...
const data = computed<HeaderData | null>(() => {
  ...
  if (properties.viewModel.kind === "navigation") return {
...
<template>
  <header v-if="data" class="fashion-header">
    ...
    <div class="fashion-nav-shell">
      <div class="fashion-nav-meta" aria-hidden="true"><span>⌖ Find stores</span><span>◎ 100k followers</span></div>
      <NuxtLink class="fashion-brand" to="/" aria-label="Mode Life home"><span>ML</span>{{ data.brand }}</NuxtLink>
      ...
      <div class="fashion-nav-actions" aria-label="Store utilities">
        <button type="button" aria-label="Search">⌕</button><button type="button" aria-label="Account">♙</button><button type="button" aria-label="Preview bag">▢<sup>0</sup></button>
      </div>
      <details class="fashion-mobile-menu">
        <summary>Menu</summary>
        <nav aria-label="Mobile navigation"><a v-for="item in data.navigation" :key="item" href="#fashion-categories">{{ item }}</a></nav>
      </details>
    </div>
  </header>
</template>
