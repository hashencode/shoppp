...
function handleDocumentKeydown(event: KeyboardEvent): void {
  if (event.key === "Escape" && searchOpen.value) void closeSearch(true);
...
async function handleInternalNavigation(event: MouseEvent): Promise<void> {
  ...
  if (dropdownToggle && innerWidth < 992) {
    event.preventDefault();
  ...
  if (!anchor || !document.body.classList.contains("fashion-store-home")) return;
  event.preventDefault();
  event.stopImmediatePropagation();
...
useHead(() => ({
  bodyAttrs: {
    class: searchOpen.value ? "fashion-store-home show-search-popup" : "fashion-store-home",
  ...
  link: [
    ...
    },
    {
      as: "image",
      fetchpriority: "high",
      href: properties.resolveAsset("fashion-store.slider-01"),
      rel: "preload",
    },
  ],
}));
      ...
      <div class="row align-items-center pt-35px pb-35px">
        ...
        <div class="col">
          <ul class="footer-navbar text-center text-md-end">
            <li class="nav-item"><a href="/" data-fashion-store-route class="nav-link">Home</a></li>
            {{
              sourceInlineGap
            }}
